...
function buildLeadPayload(){
  ...
  return {
    ...
    company:$("f-company").value.trim(), email:$("f-email").value.trim(), phone:normPhone($("f-phone").value),
    ...
    selected_pkg_name: (function(){var t=PRICING.tiers.filter(function(x){return x.key===state.selectedPkg;})[0];return t?t.name:"";})(),
    inputs_json: JSON.stringify(state.values)
  };
}
function pricingSummaryText(){
  var el=$("pricingTotal"); if(!el) return "";
  var lines=["YOUR SELECTED PLAN & INVESTMENT",""];
  var rows=el.querySelectorAll(".pt-row");
  for(var i=0;i<rows.length;i++){ var lab=rows[i].querySelector(".pt-lab"), val=rows[i].querySelector(".pt-val"); if(lab&&val) lines.push(lab.textContent.replace(/\s+/g," ").trim()+": "+val.textContent.trim()); }
  lines.push(""); lines.push("Prepared with the RSD Always-On Revenue Calculator - calculate.rsddirect.com");
  return lines.join("\n");
}
function firePricingEmail(){
  try{
    var pp=buildLeadPayload(); var txt=pricingSummaryText(); pp.pricing_summary=txt;
    if(pp.email && validEmail(pp.email) && txt && txt!==state._lastPricingSent){
      sendEmails(pp, txt); pushGHL(pp); state._lastPricingSent=txt;
    }
  }catch(e){ console.error("pricing email error",e); }
}
function sendEmails(p, pricingText){
  try{
    emailjs.init({ publicKey: RSD_CONFIG.EMAILJS.PUBLIC_KEY });
    var defaultPricing="Build your plan on the pricing page and tap 'Save / Download PDF' to get your full investment breakdown emailed to you.";
    var tp={
      to_name:p.first_name, to_email:p.email,
      client_first:p.first_name||p.name, client_email:p.email,
      from_name:"Real Space Digital",
      reply_to:RSD_CONFIG.EMAILJS.RSD_EMAIL,
      cc_email:RSD_CONFIG.EMAILJS.RSD_EMAIL,
      bcc_email:RSD_CONFIG.EMAILJS.RSD_EMAIL,
      ...
      pricing_summary:(pricingText&&pricingText.trim())?pricingText:defaultPricing,
      plan_name:p.selected_pkg_name||"",
...
function naFmt(d){ var o="+1"; if(d.length>0) o+=" ("+d.slice(0,3); if(d.length>3) o+=") "+d.slice(3,6); else if(d.length===3) o+=") "; else return o; if(d.length>6) o+="-"+d.slice(6,10); return o; }
function fmtPhone(v){ v=String(v||""); var hasPlus=v.indexOf("+")>-1; var d=v.replace(/\D/g,""); if(!hasPlus){ if(d.charAt(0)==="1") d=d.slice(1); return naFmt(d.slice(0,10)); } if(d.charAt(0)==="1"){ return naFmt(d.slice(1,11)); } var cc=d.slice(0,2), nat=d.slice(2,15); var s="+"+cc; if(nat.length) s+=" "+nat.replace(/(\d{3,4})(?=\d)/g,"$1 ").trim(); return s.trim(); }
function normPhone(v){ var hasPlus=/\+/.test(String(v)); var d=String(v).replace(/\D/g,""); if(!d) return ""; if(!hasPlus){ if(d.charAt(0)!=="1") d="1"+d; d=d.slice(0,11); return "+"+d; } return "+"+d; }
function validatePhone(v){ var hasPlus=/\+/.test(v); var d=String(v).replace(/\D/g,""); if(!hasPlus){ if(d.charAt(0)==="1") d=d.slice(1); return d.length===10; } if(d.charAt(0)==="1"){ return d.slice(1).length===10; } return d.length>=8 && d.length<=15; }
...
function hardenForm(){
  ...
  var ph=$("f-phone"); if(ph){ ph.setAttribute("inputmode","tel"); ph.setAttribute("maxlength","25"); ph.addEventListener("input", function(){ this.value=fmtPhone(this.value); }); }
...
function validateForm(){
  ...
  if(phone){ if(!validatePhone(phone)) return {ok:false,msg:"Please enter a valid phone number (e.g. +1 (434) 215-4276), or leave it blank."}; }
  return {ok:true};
}
...
document.addEventListener("DOMContentLoaded", function(){
  buildIntro();
  $("bookCall").href=RSD_CONFIG.CTA_URL; $("bookCall").addEventListener("click", firePricingEmail);
  ...
  $("toAudit2").addEventListener("click", function(){ firePricingEmail(); go("screen-audit",4); });
  $("auditBackPricing").addEventListener("click", function(){ buildPricing(); go("screen-pricing",4); });
  document.addEventListener("keydown", function(e){ if(e.key==="Escape") closeModal(); });
  wireNav(); hardenForm();
  var _pdf=$("downloadPdf"); if(_pdf) _pdf.addEventListener("click", function(){ firePricingEmail(); window.print(); });
